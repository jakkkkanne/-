...
- 楽天の公開ランキングページ(`ranking.rakuten.co.jp` 等)を直接開く/取得する
  方法も試しましたが、こちらもAkamaiのボット対策により403でブロックされ、
  この実行環境からは閲覧できませんでした(APIのIP許可リストと同じ根本原因です)。

それまでは `affiliate.PRODUCT_CATALOG` の手動キュレーションデータ(口コミ数・
口コミ内容も例示)で代替しています。

**現状の暫定リサーチ方針:検索エンジン経由での「大まかな商品名の当たりをつけ」。**
ランキングページ・APIどちらも直接アクセスできないため、当面はWebSearchツールで
「楽天市場 ランキング [ジャンル] 人気」のように検索し、スニペットに出てくる
ブランド・商品名を手がかりとして拾う方法を使います。ただし検索結果はページの
断片的な要約でしかなく、**現在の順位・正確な口コミ数・価格は確認できていません**。
そのため `PRODUCT_CATALOG` の各エントリには `brand_candidates`(見つかった候補、
価格帯外や商品カテゴリ不一致のものは除外理由をコメントで明記)というフィールドを
追加していますが、`name`/`price_jpy` などの実際に投稿へ使われる値へは、商品ページで
人が確認するまで反映していません。正確なランキングが必要になったら、
`rakuten_ranking.py`(固定IP環境での実行)か、人によるスクリーンショット共有に
切り替えます。
